...
public:
  AppAttribs(int width, int height)
      : m_FrameBuffWidth(width), m_FrameBuffHeight(height), m_MenuMode(false),
        m_FirstMouse(false), m_EscapePressed(false), m_PrevMouseX(400),
        m_PrevMouseY(300), m_DeltaTime(0) {}
  ...
  inline void SetEscaped(bool escaped) { m_EscapePressed = escaped; }
  ...
  inline double GetPrevMouseX() { return m_PrevMouseX; }

  inline double GetPrevMouseY() { return m_PrevMouseY; }

  inline void InvertMenuModeStatus() { m_MenuMode = !m_MenuMode; }

  inline void SetDeltaTime(double dt) { m_DeltaTime = dt; }
