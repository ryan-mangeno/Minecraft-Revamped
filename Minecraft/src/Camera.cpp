...
Camera::Camera(glm::vec3 position, int width, int height, float fov,
    ...
    : m_Width(width), m_Height(height), m_Position(position),
      m_PositionUpdate(position), m_PrevPosition(position),
...
void Camera::DispatchKeyboardEvent(Direction dir, float deltaTime) {
  ...
  switch (dir) {
  case NORTH:
    m_PositionUpdate += -backward * velocity;
    break;
  case SOUTH:
    m_PositionUpdate += backward * velocity;
    break;
  case WEST:
    m_PositionUpdate += -m_Right * velocity;
    break;
  case EAST:
    m_PositionUpdate += m_Right * velocity;
    break;
  case UP:
    m_PositionUpdate += g_Up * velocity;
    m_Velocity = 5.0f;
    break;
  case DOWN:
    m_PositionUpdate += -g_Up * velocity;
...
void Camera::OnUpdate(float deltaTime) {
  ...
  auto queryHits = [this]() -> std::vector<ColliderResult> {
    std::vector<glm::vec3> blocks =
        BroadPhase(glm::floor(m_PositionUpdate - 2.0f),
                   glm::ceil(m_PositionUpdate + 4.0f));
    glm::vec3 minBoxPos{m_PositionUpdate.x - PLAYER_WIDTH / 2.f,
                        m_PositionUpdate.y - PLAYER_HEIGHT,
                        m_PositionUpdate.z - PLAYER_WIDTH / 2.0f};
